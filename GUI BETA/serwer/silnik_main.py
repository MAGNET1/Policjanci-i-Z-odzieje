...
if __name__ == '__main__':
  ...
  printState['board'] = copy.deepcopy(currentBoard)
  printState['TMax'] = silnik_board.T
  printState['t'] = 1
  printState['winner'] = -1
  printState['points'] = -1
  printState['ctSequence'] = sharedInfo['ctSequence']
  printState['ttSequence'] = sharedInfo['ttSequence']
  ...
  silnik_board.constructBoard(silnik_board.wallTab, silnik_board.gateTab, silnik_board.obstacleTab, silnik_board.thiefu, silnik_board.policemanTab, False)

  start_new_thread(gui.gameLoop, (printState, 0))
  ...
  '''#### MAIN LOOP ####'''
  while True:
    # we get player moves. If they didn't make it on time, I zeroe the tab
    if sharedInfo['ttThread'] is False:
      ttSequence = sharedInfo['ttSequence']['sequence']
    else:
      print('Zlodziej nie wyrobil sie na czas!')
      ttSequence = [0] * silnik_board.k

    # same thing with policemans...
    ctSequence = []
    ...
    if sharedInfo['ctThread'] is True:
      print('Policjanci nie wyrobili sie na czas!')
    ...
    for i in range(silnik_board.k):
      ...
      if boardResult[0] != -1:
        ...
        printState['winner'] = boardResult[0]
        sharedInfo['score'] = boardResult[1]
        printState['points'] = boardResult[1]
        sharedInfo['ttThread'] = True
        sharedInfo['ctThread'] = True
